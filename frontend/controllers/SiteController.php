...
class SiteController extends Controller
{
    ...
    public function actionPeoplegetmail() {



        $model = UploadModelsUsers::find()
            ->select(['email','validts','username'])
            ->from('users')
            ->where(['confirmed' => '1'])
            ->all();
        foreach ($model as $date) {

            if ($date->validts < (time()+259200) ){

                    Yii::$app->mailer->compose()
                        ->setFrom('worktestmail111@yandex.ru')
                        ->setTo($date->email)
                        ->setSubject('Subscription is expiring soon')
                        ->setTextBody($date->username . ', your subscription is expiring soon')
                        ->send();
